...
struct node* searchforleaf(struct node* root, int k,  
                     struct node* parent, int chindex,const std::unique_ptr <opentracing::Span> & parentSpan) 
{ 
    if (root) {
        auto search_span = opentracing::Tracer::Global() -> StartSpan(
            "search", {
        ...
        // implying there are one or more children 
          ...
          /*If passed root's initial key is itself g 
            ...
            if (k < root->key[0]) 
                root = searchforleaf(root->child[0], k, root, 0,search_span); 
            ...
            { 
                ...
                for (i = 0; i < root->n; i++) 
                    if (root->key[i] > k) 
                        root = searchforleaf(root->child[i], k, root, i,search_span); 
...
struct node* insert(struct node* root, int k,const std::unique_ptr <opentracing::Span> & parentSpan) 
{ 
    if (root) { 
        ...
        struct node* p = searchforleaf(root, k, NULL, 0,insert_span); 
        struct node* q = NULL; 
        int e = k; 
    ...
    void setUpTracer(const char * configFilePath) {
        ...
        auto tracer = jaegertracing::Tracer::make(
            "btree-service", config, jaegertracing::logging::consoleLogger());
...
int main(int argc, char * argv[]) {
    ...
    // Insert 6 
    auto bspan = opentracing::Tracer::Global() -> StartSpan("BTree");
    root = insert(root, 6,bspan); 
      
    // Insert 1, 2, 4 to the left of 6 
    root->child[0] = insert(root->child[0], 1,bspan); 
    root->child[0] = insert(root->child[0], 2,bspan); 
    root->child[0] = insert(root->child[0], 4,bspan); 
    root->child[0]->parent = root; 
      
      
    // Insert 7, 8, 9 to the right of 6 
    root->child[1] = insert(root->child[1], 7,bspan); 
    root->child[1] = insert(root->child[1], 8,bspan); 
    root->child[1] = insert(root->child[1], 9,bspan); 
